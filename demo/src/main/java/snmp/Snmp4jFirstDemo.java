...
public class Snmp4jFirstDemo {
    ...
    public void sendMessage(Boolean sync, final Boolean bro, PDU pdu, String addr) throws IOException {
	// 生成目标地址对象
	...
	if (!sync) {
	     ...
	     * request(发送的请求):GET[requestID=632977521, errorStatus=Success(0), errorIndex=0,
	     * VBS[1.3.6.1.2.1.1.5.0 = Null]] response(返回的响应):RESPONSE[requestID=632977521,
	...
	try {
	    ...
	    // "udp:192.168.1.229/161");//192.168.1.229 Linux服务器
	    manager.sendMessage(false, true, pdu, "udp:10.1.69.81/161");// 192.168.1.233 WinServer2008服务器
	} catch (IOException e) {
	    e.printStackTrace();
	}
    }
